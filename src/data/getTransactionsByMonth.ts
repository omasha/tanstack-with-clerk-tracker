...
export const getTransactionsByMonth = createServerFn({
  ...
  .handler(async ({ context, data }) => {
    const earliestDate = new Date(data.year, data.month - 1, 1); // first day of the month
    const latestDate = new Date(data.year, data.month, 0); // zero indicates the last day of the previous month!
